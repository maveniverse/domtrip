...
public class Editor {
    ...
    public Comment commentOutElement(Element element) throws DomTripException {
        ...
        // Serialize the element to XML, but strip its preceding/following whitespace
        // since we'll add our own spaces around the comment content
        String elementXml = getElementXmlWithoutOuterWhitespace(element);
    ...
    public Comment commentOutElements(Element... elements) throws DomTripException {
        ...
        for (Element element : elements) {
            ...
            commentContent.append(getElementXmlWithoutOuterWhitespace(element));
        }
        ...
        // For multiple elements, we need to preserve the whitespace structure
        // The comment should have the following whitespace of the last element
        // plus an additional blank line to separate it from the next element
        String lastElementFollowing = elements[elements.length - 1].followingWhitespace();

        // Check if there's an element after the last commented element
        Element lastElement = elements[elements.length - 1];
        int lastElementIndex = findElementIndex(parent, lastElement);
        if (lastElementIndex >= 0 && lastElementIndex + 1 < parent.nodeCount()) {
            Node nextNode = parent.getNode(lastElementIndex + 1);
            if (nextNode instanceof Element nextElement) {
                // For multiple elements, add a blank line after the comment
                // This preserves the visual separation that was between the elements
                String indentation = whitespaceManager.inferIndentation(parent);
                // Create the following whitespace: newline + indentation for blank line + newline
                comment.followingWhitespace(lineEnding + indentation + lineEnding);

                // Ensure the next element has proper preceding whitespace
                // Since we added a blank line in the comment's following whitespace,
                // the next element just needs indentation (no newline)
                nextElement.precedingWhitespaceInternal(indentation);
            } else {
                comment.followingWhitespace(lastElementFollowing);
            }
        } else {
            comment.followingWhitespace(lastElementFollowing);
        }
            ...
            if (hasTextNodeBefore && index == parent.nodeCount()) {
                ...
                if (index == 0) {
                    ...
                    if (prevFollowing.isEmpty()) {
                        ...
                        // But if we're inserting at the end, we need to ensure proper indentation
                        if (index == parent.nodeCount()) {

                            // For end insertion, ensure the new element gets proper indentation
                            newElement.precedingWhitespaceInternal(lineEnding + indentation);
                            // Clear previous element's following whitespace
                            prevElement.followingWhitespaceInternal("");
                        } else {

                            // Normal case - transfer whitespace
                ...
                // If inserting at the end, ensure proper following whitespace and preceding whitespace
                if (index == parent.nodeCount()) {
                    // For end insertion, ensure we have proper indentation
    ...
    public Element insertElementAfter(Element referenceElement, String elementName) throws DomTripException {
        ...
        if (!indentation.isEmpty()) {
            ...
            if (hasTextNodeAfter) {
                ...
                // Inserting at the end - handle reference element's following whitespace
                String referenceFollowing = referenceElement.followingWhitespace();
                if (!referenceFollowing.isEmpty()) {
                    // Clear the reference element's following whitespace to avoid duplication
                    referenceElement.followingWhitespaceInternal("");
                }
                // Check if we need to format the reference element
                if (referenceElement.precedingWhitespace().isEmpty()) {
                    // This is compact XML, format the reference element too
                    referenceElement.precedingWhitespaceInternal(lineEnding + indentation);
                }
                newElement.precedingWhitespaceInternal(lineEnding + indentation);
                // Use the reference element's original following whitespace for proper parent closing tag indentation
                if (!referenceFollowing.isEmpty()) {
                    newElement.followingWhitespaceInternal(referenceFollowing);
                } else {
                    newElement.followingWhitespaceInternal(lineEnding);
                }
            }
    ...
    /**
     * Gets the XML representation of an element without its preceding and following whitespace.
     * This preserves the internal formatting but removes the outer whitespace that would
     * interfere with comment formatting.
     */
    private String getElementXmlWithoutOuterWhitespace(Element element) {
        // Temporarily store the original whitespace
        String originalPreceding = element.precedingWhitespace();
        String originalFollowing = element.followingWhitespace();

        // Temporarily clear the outer whitespace
        element.precedingWhitespaceInternal("");
        element.followingWhitespaceInternal("");

        // Get the XML without outer whitespace
        String xml = element.toXml();

        // Restore the original whitespace
        element.precedingWhitespaceInternal(originalPreceding);
        element.followingWhitespaceInternal(originalFollowing);
