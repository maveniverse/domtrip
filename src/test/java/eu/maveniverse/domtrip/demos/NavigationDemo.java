...
public class NavigationDemo {
    ...
    private static void demonstrateBasicNavigation(Editor editor) {
        ...
        root.findChild("metadata").ifPresent(metadata -> {
            ...
            System.out.println("Established: " + 
                metadata.findChild("established").map(Element::getTextContent).orElse("Unknown"));
        });
        ...
        root.findChild("sections").ifPresent(sections -> {
            ...
            sectionList.forEach(section -> {
                ...
                String name = section.findChild("name").map(Element::getTextContent).orElse("Unknown");
    ...
    private static void demonstrateStreamNavigation(Editor editor) {
        ...
        List<Element> allBooks = root.descendants()
        ...
        availableBooks.forEach(book -> {
            ...
            String author = book.findChild("author").map(Element::getTextContent).orElse("Unknown");
    ...
    private static void demonstrateAdvancedQueries(Editor editor) {
        ...
        root.descendants()
            ...
            .forEach(book -> {
                String title = book.findChild("title").map(Element::getTextContent).orElse("Unknown");
                String year = book.findChild("year").map(Element::getTextContent).orElse("Unknown");
                String author = book.findChild("author").map(Element::getTextContent).orElse("Unknown");
